...
(define (draw-texture texture-obj x y #!key (width #f) (height #f))
  (let* ((tex-id      (texture-id texture-obj))
         ...
         (x           (fixnum->flonum x))
         (y           (fixnum->flonum y))
         (width       (fixnum->flonum (if width  width  tex-width)))
         (height      (fixnum->flonum (if height height tex-height)))
         (tex-x-max   (/ width  tex-width))
         (tex-y-max   (/ height tex-height)))
    (setup-texture-render-env tex-id)
    (glBegin GL_QUADS)
    (begin
      (glTexCoord2f 0.0       0.0)       (glVertex2f x           y)
      (glTexCoord2f 0.0       tex-y-max) (glVertex2f x           (+ y height))
      (glTexCoord2f tex-x-max tex-y-max) (glVertex2f (+ x width) (+ y height))
      (glTexCoord2f tex-x-max 0.0)       (glVertex2f (+ x width) y))
    (glEnd)
